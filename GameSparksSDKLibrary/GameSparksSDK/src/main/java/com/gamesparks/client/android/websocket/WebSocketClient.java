package com.gamesparks.client.android.websocket;
...
/**
 * @see https://github.com/codebutler/android-websockets
 */
public class WebSocketClient {
    ...
    public WebSocketClient(URI uri, Listener listener, List<BasicNameValuePair> extraHeaders) {
        ...
        mHandlerThread = new HandlerThread("websocket-sender-thread");
        ...
        mThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.d(TAG, "WebSocket EOF! Calling onDisconnect");
                    mListener.onDisconnect(0, "EOF");

                } catch (SSLException ex) {
                    // Connection reset by peer
                    Log.d(TAG, "Websocket SSL error! Calling onDisconnect", ex);
                    mListener.onDisconnect(0, "SSL");

                } catch (Exception ex) {
                    mListener.onError(ex);
                } finally {
                    Log.d(TAG, "mThread has stopped");
                    try {
                        if (mSocket != null) {
                            Log.d(TAG, "mSocket closed in finally block");
                            mSocket.close();
                        }
                    } catch (Exception e) {
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        if (mSocket != null) {
                            Log.d(TAG, "mSocket closed in disconnect");
                            mSocket.close();
                        }
                    } catch (Exception ex) {
    ...
    protected void finalize() throws Throwable {
        ...
        try {
            if (mSocket != null) {
                Log.d(TAG, "mSocket closed in finalize");
                mSocket.close();
            }
        } catch (Exception ex) {
